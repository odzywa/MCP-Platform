...
from drivers.docker_driver import DeploySpec, DockerDeploymentDriver
...
def dispatch_event(event: str, runtime_id: str, details: dict) -> None:
    payload = json.dumps({"event": event, "runtime_id": runtime_id,
                          "timestamp": now_sql(), "details": details}).encode()
    def _fire() -> None:
        try:
...
BUILD_CONTEXT_ROOT = Path(os.getenv("MCP_RUNTIME_BUILD_CONTEXT_ROOT", "/workspace"))
...
def run_action(driver: DockerDeploymentDriver, conn: sqlite3.Connection, request: sqlite3.Row) -> None:
    ...
    if action in {"deploy", "redeploy"}:
        ...
        if not config_path or not Path(config_path).exists():
            raise RuntimeError(f"config path missing or not found: {config_path}")
        log(conn, runtime_id, f"Deploying runtime container from {config_path}")
        ...
        log(conn, runtime_id, f"Runtime started at {status.endpoint_url}")
        audit_json(conn, "deploy_runtime", runtime_id, {"container": status.container_name, "endpoint": status.endpoint_url})

    elif action == "rebuild_redeploy":
        image = runtime["image"]
        context_path = IMAGE_BUILD_CONTEXTS.get(image)
        if context_path is None:
            raise RuntimeError(f"no build context configured for image: {image}")
        if not context_path.exists():
            raise RuntimeError(f"build context missing for {image}: {context_path}")
        log(conn, runtime_id, f"Building runtime image {image}")
        driver.build_image(context_path, image)
        log(conn, runtime_id, f"Image rebuilt: {image}")
        config_path = runtime["config_path"]
        if not config_path or not Path(config_path).exists():
        ...
        log(conn, runtime_id, f"Runtime redeployed at {status.endpoint_url}")
        audit_json(conn, "rebuild_redeploy", runtime_id, {"image": image, "endpoint": status.endpoint_url})
        ...
        log(conn, runtime_id, "Runtime stopped")
        audit_json(conn, "stop_runtime", runtime_id, {"container": status.container_name})
        ...
        log(conn, runtime_id, "Runtime started")
        audit_json(conn, "start_runtime", runtime_id, {"container": status.container_name})
        ...
        log(conn, runtime_id, "Runtime restarted")
        audit_json(conn, "restart_runtime", runtime_id, {"container": status.container_name})
        ...
        log(conn, runtime_id, "Runtime deleted")
        ...
        audit_json(conn, "health_refresh", runtime_id, {"state": status.state, "error": status.last_error})

    elif action == "reload":
        result = driver.reload_config(runtime_id)
        conn.execute(
            "UPDATE runtimes SET last_error = NULL, updated_at = ? WHERE id = ?",
            (now_sql(), runtime_id),
        )
        log(conn, runtime_id, f"Config reloaded: {result.get('tools', '?')} tools")
        audit_json(conn, "reload_runtime", runtime_id, result)
...
def run_image_build(driver: DockerDeploymentDriver, conn: sqlite3.Connection, request: sqlite3.Row) -> None:
    build_id = request["id"]
    image = request["image"]
    context_path = Path(request["context_path"])
    if not context_path.exists():
        raise RuntimeError(f"build context missing: {context_path}")
    log(conn, build_id, f"Building runtime image {image} from {context_path}")
    driver.build_image(context_path, image)
    log(conn, build_id, f"Runtime image built: {image}")
    audit_json(conn, "build_runtime_image", build_id, {"image": image, "context": str(context_path)})


def sync_runtime_statuses(driver: DockerDeploymentDriver, conn: sqlite3.Connection) -> None:
    ...
    for runtime in runtimes:
        ...
        if runtime_id in status_by_id:
            conn.execute(
                "UPDATE runtimes SET status = ?, updated_at = ? WHERE id = ?",
                (status_by_id[runtime_id].state, now_sql(), runtime_id),
            )
        else:
            conn.execute(
                "UPDATE runtimes SET status = ?, container_name = NULL, endpoint_url = NULL, updated_at = ? WHERE id = ?",
                ("missing", now_sql(), runtime_id),
            )
            dispatch_event("health_failed", runtime_id, {"state": "missing"})


def loop() -> None:
    client = docker.from_env()
    driver = DockerDeploymentDriver(client)
    while True:
        try:
            with connect() as conn:
                ...
                for request in requests:
                    ...
                    try:
                        ...
                        dispatch_event("runtime_failed", runtime_id, {"error": str(exc), "action": request["action"]})
                ...
                for image_build in image_builds:
                    ...
                    conn.commit()
                    try:
                        run_image_build(driver, conn, image_build)
                        conn.execute(
