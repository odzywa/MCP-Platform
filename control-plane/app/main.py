...
(function(){{
  ...
  function translateText(text) {{
    for(var i=0;i<TRANS_KEYS.length;i++) {{
      if(text.indexOf(TRANS_KEYS[i]) !== -1) {{
        text = text.split(TRANS_KEYS[i]).join(TRANS_VALS[i]);
      }}
    }}
    return text;
  }}
...
def quick_start_page(error: str = "") -> str:
    ...
    packages = store.rows("SELECT id, name, description, category, risk_level, package_json FROM tool_packages WHERE enabled=1 ORDER BY category, name")
